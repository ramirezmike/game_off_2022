use crate::{game_camera::PanOrbitCamera, ingame::CleanupMarker, ui::text_size, menus, assets::GameAssets};
use bevy::prelude::*;
    ...
    for (entity, mut style, calculated, mut follow) in text_query.iter_mut() {
                            ...
                            FollowThing::Entity(e) => {
                                if let Ok(mesh_position) = mesh_query.get(e) {
                                    Vec3::new(
                                        ...
                                        mesh_position.translation.y + 3.0,
        ...
        for (camera, camera_transform) in camera_query.iter() {
            ...
            match camera.world_to_viewport(camera_transform, translation) {
                Some(coords) => {
                    style.position.left = Val::Px(coords.x).try_sub(calculated.size.width / 2.0).unwrap();
                    style.position.bottom = Val::Px(coords.y).try_sub(calculated.size.height / 2.0).unwrap();
                }
                None => {
...
//                        width: Val::Percent(50.0),
            ...
            .insert(BackgroundColor(Color::rgba(0.0, 0.0, 0.0, 0.7)))
